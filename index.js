...
let clientDeviceId = 'unknown-device';
...
const ensureUserDoc = async (user, email, ip, deviceId, isRegistration) => {
...
const validateBanAfterLogin = async (userRef) => {
  ...
  if ((await isIpBlocked(data.lastIp || clientIp)) || (await isDeviceBlocked(data.lastDeviceId || clientDeviceId))) {
...
const authFlow = async (action, progressText, isRegistration) => {
  ...
  try {
    ...
    const userRef = await ensureUserDoc(result.user, creds.email, clientIp, clientDeviceId, isRegistration);
...
const boot = async () => {
  ...
  clientDeviceId = await getClientDeviceId();
  if ((await isIpBlocked(clientIp)) || (await isDeviceBlocked(clientDeviceId))) {
